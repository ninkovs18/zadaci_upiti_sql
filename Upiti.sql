...
select *
from Article a
where a.ExpireDate BETWEEN '2018-09-01' AND '2018-12-31'
...
from					Article a 
where					a.ExpireDate BETWEEN '2019-01-01' AND '2020-12-31'
...
select top 100 *
from Firm f
where f.Email is not null and LEN(f.Email) > 0
...
select top 100 *
from Article a
where DATEADD(MONTH, -3, GETDATE()) <= a.ExpireDate and GETDATE() >= a.ExpireDate

--21. Prikazati sve Artikle koji imaju ExpireDate u poslednjih 30 dana od trenutka izvrsenja querija
select top 100 * 
from Article a
where DATEADD(DAY, -30, GETDATE()) <= a.ExpireDate and GETDATE() >= a.ExpireDate
...
select top 100 *
from Article a
where a.Location = 'Mexico' and a.NumberOfVisits > 100
